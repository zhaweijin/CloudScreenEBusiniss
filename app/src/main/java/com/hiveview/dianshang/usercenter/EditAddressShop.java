...
public class EditAddressShop extends BaseFragment {
    ...
    private String tag="EditAddressShop";
    private int id;
    private int MAXLENG=40;
    private int MAXLENGRE=16;
    private SharedPreferences preferencesreceiveID;

    @Override
    protected int getLayoutId() {
        return R.layout.edit_address_shop;
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        preferencesreceiveID = getActivity().getSharedPreferences("receiveID",
                getActivity().MODE_WORLD_READABLE);
    ...
    private void initEditAddressUI(){
        ...
        region.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if(hasFocus){
                    //关闭软键盘
                    View view = ((EditAddressActivity)mContext).getWindow().peekDecorView();
                    if (view != null) {
        ...
        detailedAddress.addTextChangedListener(new TextWatcher() {
            ...
            @Override
            public void afterTextChanged(Editable s) {
                int lines = detailedAddress.getLineCount();
                // 限制最大输入字数  40
    ...
    public void addUserAddress(UserData userData, boolean isDefault){
        ...
        try{
            ...
            Log.i("========","===format=="+format(userData.getAddressDetail()));
                ...
                .subscribe(new Subscriber<StatusData>() {
                    ...
                    public void onNext(StatusData statusData) {
                        ...
                        if(statusData.getReturnValue()==0){
                            Intent intent = new Intent("com.dianshang.damai.message_finish");
                            mContext.sendBroadcast(intent);
                            int id=preferencesreceiveID.getInt("receiveID",-1);
                            if(id!=-1){
                                UserAdderssSelect.launch((EditAddressActivity)mContext,true,id);
                            }else{
                                UserAdderssSelect.launch((EditAddressActivity)mContext,false,id);
                            }

                            ((EditAddressActivity) mContext).finish();
        ...
        public boolean onKey(View v, int keyCode, KeyEvent event) {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                if(v.getId()==R.id.detailedAddress && keyCode==KeyEvent.KEYCODE_DPAD_DOWN) {
                    ...
                    detailedAddress.setNextFocusRightId(R.id.detailedAddress);
                    ...
                    recipient.setNextFocusLeftId(R.id.recipient);
                }else if(v.getId()==R.id.region && keyCode==KeyEvent.KEYCODE_DPAD_LEFT ){
                    region.setNextFocusLeftId(R.id.region);
                }else if(v.getId()==R.id.street && keyCode==KeyEvent.KEYCODE_DPAD_LEFT ){
                    street.setNextFocusLeftId(R.id.street);
    ...
    BroadcastReceiver nettyReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (isUpdate && domyTcpMsgBodyVo.getInfo().get(0).getActionType() == ConStant.ACTION_TYPE_MODITY_ADDRESS && domyTcpMsgBodyVo.getInfo().get(0).getContentId().equals(ConStant.NETTY_OP_SUCESS)) {  //修改地址
                ...
                UserAdderssSelect.launch((EditAddressActivity)mContext);
                ((EditAddressActivity) mContext).finish();
            } else if (!isUpdate && domyTcpMsgBodyVo.getInfo().get(0).getActionType() == ConStant.ACTION_TYPE_ADD_ADDRESS && domyTcpMsgBodyVo.getInfo().get(0).getContentId().equals(ConStant.NETTY_OP_SUCESS)) {
